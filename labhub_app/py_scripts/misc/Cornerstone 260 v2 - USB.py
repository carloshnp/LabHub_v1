...
print("Conectando ao monocromador")
clr.AddReference("Cornerstone")
import CornerstoneDll
MONO = CornerstoneDll.Cornerstone(True)
if not MONO.connect():
...
print("############# IMPORTANTE!!!  #############")
print("O motor da saída monocromador foi fisicamente desconectado porque a correia está destruída.")
print("Isso precisa ser resolvido!!! Criando bypass por agora.")
print('Comando MONO.sendCommand("OUTPORT 2") coloca o espelho para porta de saída correta.')
print("########################################")
print("Monocromador conectado")


#GPIB do Lock in SR530 tem que estar ajustado para 23
print("Conectando ao lock-in")
...
###########
    ...
    def StatusButtonColor(self):
        ...
        if(Fim == 1):
            ...
            self.Status_txt.setText("Finalizado")
        if(Pausa == 1):
...
##########
    ...
    def GOTO(self):
        ...
        if isinstance(Posicao, float):
            MONO.setWavelength(round(Posicao))
            # time.sleep(wait_time_monocromador)
            self.AtualizaPosicao()
        return
    ...
    def PuloMais(self):
        try:
            ...
            PosicaoAtual = MONO.getWavelength() / 1000
            comando = str(round(PosicaoAtual + Pulo))
            MONO.setWavelength(comando)
            # time.sleep(wait_time_monocromador)
        except:
            return
        self.AtualizaPosicao()
